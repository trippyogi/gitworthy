const SENSITIVE_HEADER = /^(authorization|proxy-authorization|cookie|set-cookie|x-api-key|x-access-token|x-npm-token|x-request-id|x-github-request-id|traceparent|tracestate)$/i;
const SENSITIVE_PARAM = /^(access_token|x-access-token|token|auth|authorization|api_key|private_key|client_secret|password|request_id|request-id|x-request-id)$/i;
const SENSITIVE_KEY = /(token|authorization|cookie|secret|password|api[_-]?key|private[_-]?key|request[_-]?id|x-github-request-id)/i;
...
export function scrubSecretText(value: string): string {
  return value
    ...
    .replace(/(cookie\s*[:=]\s*)[^\n\r;,"']+/gi, '$1[redacted]')
    .replace(/((?:access_token|x-access-token|token|api_key|private_key|client_secret|password|request[_-]?id)\s*[:=]\s*)[^\s,&"']+/gi, '$1[redacted]')
    .replace(/\b(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{20,}\b/g, '[redacted]')
    .replace(/\bgithub_pat_[A-Za-z0-9_]{20,}\b/g, '[redacted]')
    .replace(/\bnpm_[A-Za-z0-9_]{16,}\b/g, '[redacted]')
...
function looksTokenLike(value: string): boolean {
  return /\b(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{20,}\b/.test(value)
    ...
    || /\bnpm_[A-Za-z0-9_]{16,}\b/.test(value)
