...
const CONCURRENCY = 2;
const ORG_SCAN_LIMIT = 'org_scan ranks candidates across multiple repositories by tracker quality only; it does not replace per-repo contrib_policy, duplicate, or in-flight checks before opening a PR.';
...
function rankRepos(repos: GithubRepoSummary[]): GithubRepoSummary[] {
  return repos
    .filter((repo) => !repo.archived && !repo.fork)
    .sort((left, right) => {
      const starDiff = (right.stargazers_count ?? 0) - (left.stargazers_count ?? 0);
      if (starDiff !== 0) return starDiff;
      const leftDate = Date.parse(left.pushed_at ?? left.updated_at ?? '') || 0;
      const rightDate = Date.parse(right.pushed_at ?? right.updated_at ?? '') || 0;
...
export async function org_scan(input: Input): Promise<Envelope> {
  ...
  const checked = [
    ...
    `ranked ${ranked.length} eligible repositor${ranked.length === 1 ? 'y' : 'ies'} by stargazers_count then recent push activity`,
